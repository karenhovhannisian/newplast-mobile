...
const Filters = ({products}) => {
  ...
  const changeFilter = (categoryIndex, subIndex) => {
    ...
    copyCategories[categoryIndex].subCategories[subIndex].checked = !categories[
      categoryIndex
    ].subCategories[subIndex].checked;
    ...
    <View style={styles.container}>
          ...
          return (
            <View style={styles.bodyView}>
              <TouchableOpacity
                onPress={() =>
                  setActiveIndex(
                    activeIndex === categoryIndex ? null : categoryIndex,
                  )
                }
                key={category.name}
                style={styles.filterCategoriesView}>
                  ...
                  {category.subCategories.map((el, index) => {
                    return (
                      <View
                        ...
                        <Text
                          onPress={() => changeFilter(categoryIndex, index)}
                          style={styles.subCategoriesText}
                          key={el.name}>
                          {el.name}
                        </Text>
                      </View>
      ...
      <View style={styles.filterSubmitView}>
        <TouchableOpacity style={styles.touchableButton}>
          <Text style={styles.filterSubmit}>Հաստատել</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
